<script lang="ts">
    ...
    onMount(() => {
        ...
        const routes = ['/api/v1/auth/login','/api/v1/users','/api/v1/games','/api/v1/messages','/api/v1/tickets'];
        const users = ['alice','baptiste','caroline','daniel','eve',''];
        const reasons = [null,'invalid_credentials','forbidden','not_found','rate_limited'];

        const now = Date.now();
        for (let i=0;i<80;i++){
            const lvl: Log['level'] = i%7===0 ? 'error' : (i%3===0 ? 'warn' : 'info');
            const d = new Date(now - Math.floor(Math.random()*48)*3600*1000 - Math.floor(Math.random()*3600*1000));
            const route = routes[Math.floor(Math.random()*routes.length)];
            const method = methods[Math.floor(Math.random()*methods.length)];
            const status = lvl === 'error' ? (Math.random()>0.5?401:500) : (Math.random()>0.8?500:200+(Math.random()>0.5?0:0));
            const username = users[Math.floor(Math.random()*users.length)];
            const reason = lvl !== 'info' ? reasons[Math.floor(Math.random()*reasons.length)] : null;
            const message = lvl !== 'info' && reason ? reason : `Example message #${i+1}`;
            logs.push({ id:i+1, date: d.toISOString(), level: lvl, route, message, username: username||undefined, method, status, reason: reason||undefined, raw: { username, method, status, reason } });
        }
        filtered = logs.slice();
        sortAndPaginate();
        loading = false;
