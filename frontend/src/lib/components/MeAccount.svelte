...
<div class="rounded-2xl bg-white/90 backdrop-blur-md ring-1 ring-black/5 shadow-lg p-4">
    ...
    <div class="flex items-center gap-4 mb-4">
        <div class="w-14 h-14 bg-gray-200 rounded-full flex items-center justify-center text-gray-500 text-xl font-semibold">
            {initials((user as any)?.username)}
        </div>
        <div>
    ...
    <div class="space-y-3">
    <button onclick={goToLogin} class="w-full bg-gray-100 text-gray-800 py-3 rounded-lg text-sm">Changer le mot de passe</button>
    <button onclick={doLogout} class="w-full bg-red-100 text-red-800 py-3 rounded-lg text-sm">Se déconnecter</button>
    </div>
</div>
