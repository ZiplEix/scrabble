...
<div class="relative rounded-2xl ring-1 ring-black/5 bg-white shadow p-4 flex flex-col gap-2">
	<a href={`/games/${game.id}`} class="flex flex-col gap-1">
		<h2 class="text-base font-semibold text-gray-900 truncate">{game.name}</h2>
		{#if showLastPlayTime}
			<p class="text-[11px] text-gray-500">Dernier coup : {formatDate(game.last_play_time)}</p>
		{/if}
		{#if showTurnOf}
			<p class="text-[12px] text-gray-700">Tour de : <span class="font-medium">{game.current_turn_username}</span></p>
		{/if}
		{#if winning}
			<p class="text-[12px] text-gray-700">Gagnant:
	...
	{#if game.is_your_game}
		<div class="absolute top-2 right-2">
			...
			{#if menuOpen}
				...
				>
					<button class="block w-full px-4 py-2 text-left hover:bg-gray-100" onclick={renameGame}>
						Renommer
					</button>
					<button class="block w-full px-4 py-2 text-left text-red-600 hover:bg-gray-100" onclick={deleteGame}>
						Supprimer
					</button>
				</div>
			{/if}
